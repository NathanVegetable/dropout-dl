...
namespace dropout_dl {
	...
	bool episode::is_special_episode(const std::string& episode_url) {
		...
		return episode_url.find("behind-the-scenes") != std::string::npos ||
		       ...
		       episode_url.find("last-looks") != std::string::npos ||
		       episode_url.find("bonus-content") != std::string::npos;
	}
	...
	void episode::download_quality(const std::string& quality, const std::string& base_directory, const std::string& filename, const std::string& audio_quality, const std::string& container_format, int segment_buffer_size) {
		...
		// Select audio quality based on preference
		// audio_qualities typically contains: [64000, 255000, 128000]
		int audio_quality_index = 0;

		if (audio_quality == "highest") {
			// Find the highest bitrate
			int max_bitrate = 0;
			for (int i = 0; i < this->audio_qualities.size(); i++) {
				int bitrate = std::stoi(this->audio_qualities[i]);
				if (bitrate > max_bitrate) {
					max_bitrate = bitrate;
					audio_quality_index = i;
				}
			}
		}
		else if (audio_quality == "lowest") {
			// Find the lowest bitrate
			...
			// "medium" or default - find the middle bitrate
			// Sort indices by bitrate and pick the middle one
			std::vector<std::pair<int, int>> bitrate_indices; // (bitrate, index)
			for (int i = 0; i < this->audio_qualities.size(); i++) {
				bitrate_indices.push_back({std::stoi(this->audio_qualities[i]), i});
			}
			std::sort(bitrate_indices.begin(), bitrate_indices.end());
			audio_quality_index = bitrate_indices[bitrate_indices.size() / 2].second;
		}
